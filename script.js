...
class MonInstanceTemplate {
    ...
    constructor(mon, maxHp, playerNo) {
    ...
    getTempMoveStats(move, stat) {
        if (this.moveStats[move][stat])
            return this.moveStats[move][stat];
        else
            return getMoveStats(move)[stat];
    }
    $(func) {
        if (settings.mode == this.modeName)
            func();
    }
    addTempEffect(effect, { turns = Infinity, prob = 1, layers = 1 } = {}) {
        if (Math.random() < prob) {
            if (!this.tempEffect[effect])
                this.tempEffect[effect] = new TempEffectValue();
...
for (let i = 0; i < 4; i++) {
    ...
    decisionMove[i].addEventListener("mouseover", function () {
        ...
        let stats = (key) => battleInfo[playerToMove].build[battleInfo[playerToMove].currentPokemon].getTempMoveStats(decisionMoveFor, key);
...
function addListenerToInput(className, listId) {
...
function refreshDecision() {
    if (battleInfo[playerToMove].currentPokemon != -1 && getPkmn(true).uncontrollable.turns == 0) {
        for (let i = 0; i < 4; i++) {
            ...
            else if (Object.values(getPkmn(true).moves)[i] <= 0 || (getPkmn(true).disable.move == Object.keys(getPkmn(true)
                .moves)[i]))
                button.disabled = true;
            else
                button.disabled = false;
            $("roco kingdom", () => {
                if (getPkmn(true).energy < getPkmn(true).getTempMoveStats(Object.keys(getPkmn(true).moves)[i], "cost"))
                    button.disabled = true;
            });
            let tempMove = "";
            ...
            $("pokemon", () => {
                modifyDecisionMoveBtn(button, getL10n("moves", tempMove), Object.values(getPkmn(true).moves)[i].toString(), getPkmn(true).getTempMoveStats(Object
            ...
            $("roco kingdom", () => {
                ...
                button.querySelector(".move-type").innerText = getL10n("types", getPkmn(true).getTempMoveStats(tempMove, "type"));
                button.querySelector(".cost").innerText = getPkmn(true).getTempMoveStats(tempMove, "cost");
...
function displayEffectiveness(button, move) {
    ...
    if (!getPkmn(false) || !move || getPkmn(true).getTempMoveStats(move, "cat") == "status")
        button.querySelector(".effectiveness").innerText = "";
    else
        button.querySelector(".effectiveness").innerText = EFFECTIVENESS_ABBR[calculateEffectiveness(getPkmn(true).getTempMoveStats(move, "type"), getType(false))];
}
function getDefaultProperties(playersInfo) {
    ...
    for (let i of [0, 1]) {
        for (let j = 0; j < arr[i].build.length; j++)
            arr[i].build[j] = new MonInstance(arr[i].build[j], i);
        $("roco kingdom", () => {
            arr[i].marks = {
                "positive": {
                    "name": "",
...
function addTooltip(elementGroup, i, player = playerToMove) {
    ...
    addEffectBadges(battleInfo[actualPlayer].build[i], tooltip.querySelector(".tip-status"), true);
    for (let j = 0; j < 4; j++) {
        $("pokemon", () => {
        ...
        $("roco kingdom", () => {
            addMoveToTooltip(tooltip, actualPlayer, i, j, {
                "additionalInfoFn": () => {
                    tooltip.querySelectorAll(".cost")[j].innerText = battleInfo[actualPlayer].build[i].getTempMoveStats(Object.keys(battleInfo[actualPlayer]
    ...
    $("pokemon", () => {
        tooltip.querySelector(".spe-range").innerText = `${calcActualValue(stats2.spe, 0, 0, battleInfo[actualPlayer].build[i].lv)}~${calcActualValue(stats2
            .spe, 15, 252, battleInfo[actualPlayer].build[i].lv)}`;
    });
    $("roco kingdom", () => {
        tooltip.querySelector(".spe-range").innerText = `${calcActualValueRk(stats2.spe, 0)}~${calcActualValueRk(stats2.spe, 100)}`;
    });
...
function calcActualValue(base, dv, ev, lv) {
    return Math.floor(0.01 * (2 * (base + dv) + Math.floor(0.25 * ev)) * lv) + 5;
}
function calcActualValueRk(base, ev) {
    return Math.round(Math.round(base * 1.1 + ev * 0.55 + 10) + 50);
}
function addMoveToTooltip(tooltip, actualPlayer, i, j, { additionalInfoFn, emptyMoveFn, unknownMoveFn }) {
...
function addEffectBadges(pkmn, outputArea, showFull) {
...
function getDefense(isSelf, isCrit) {
    if (isCrit)
        ...
        return getPkmn(isSelf).def * STAGE_MULTIPLIER[getPkmn(isSelf).defStage] * ((getPkmn(isSelf).tempEffect.reflect.turns) ? 2 : 1);
}
...
function sortAttackFn(a, b) {
    if (b.type == "switch" && a.type == "move")
        return 1;
    else if (a.type == "switch" && b.type == "move")
        return -1;
    else if (a.type == "move" && b.type == "move") {
        let stats = (attack, stat) => battleInfo[attack.user].build[battleInfo[attack.user].currentPokemon].getTempMoveStats(attack.move, stat);
...
function nextTurn() {
    ...
    for (let i of [true, false]) {
        for (let index in getPkmn(i).tempEffect) {
            if (getPkmn(i).tempEffect[index].turns > 0)
                getPkmn(i).tempEffect[index].turns--;
        }
    }
    $("roco kingdom", () => {
    ...
    outer: for (let i of attacks) {
        ...
        for (let k of $("moves"))
            if (k.name == i.move) {
                if (k.dmgDeduction)
                    getPkmn(true).dmgDeduction = k.dmgDeduction;
                let effect;
                if (k.cat != "status" && Math.random() > k.acc * ACC_STAGE_MULTIPLIER[getPkmn(true).accStage] *
                    ...
                    if (getPkmn(false).tempEffect.semiInvulnerable.turns > 0 && !preDmgEffect.nullifySemiInvulnerable)
                        break;
...
function attack(move, preAttackInfo) {
    let k = getMoveStats(move);
    ...
    $("roco kingdom", () => {
        if (k.cat == "physical")
            dmg = calculateDmgRk(k.power, getAttack(true), getDefense(false, isCrit), k.type, getType(false), getType(true));
        else
            dmg = calculateDmgRk(k.power, getPkmn(true).spa, getPkmn(false).spd, k.type, getType(false), getType(true));
    });
...
function renderHP(info = battleInfo) {
    ...
    for (let i of [0, 1])
        if (info[Number(i != viewpoint)].currentPokemon != -1) {
            addEffectBadges(info[Number(i != viewpoint)].build[info[Number(i != viewpoint)].currentPokemon], document.getElementById(`p${i + 1}Status`), false);
        }
    refreshBalls(info);
}
...
function modifyStats(isSelf, stat, delta, prob) {
    if (getPkmn(isSelf).tempEffect.mist.turns > 0 && delta < 0)
        return;
    ...
    for (let i of document.querySelectorAll("." + name))
        i.addEventListener("blur", function () {
            ...
            if (i.dataset.valueFor)
                players[Number(i.dataset.player) - 1].build[Number(i.dataset.no) - 1][name][i.dataset.valueFor] = Number(i.innerText);
            else
...
function addMark(playerIndex, name, layers) {
    let mark;
