...
const MOVES = [{
        ...
        "effect": function (e) {
            ...
            if (e.substitutePreDmg && getPkmn(false).substituteHp <= 0)
                return;
        ...
        "effect": function (e) {
            ...
            if (num < 3 / 8)
                ...
                repeatAttack(e.totalDmg, 3);
            else
                repeatAttack(e.totalDmg, 4);
        }
    }, {
        "name": "barrier",
        "type": "psychic",
        ...
        "priority": 0,
        "effect": function () {
            if (!getType(false).includes("normal"))
        ...
        "effect": function () {
            for (let i of [true, false])
                for (let j of ["atkStage", "defStage", "spStage", "speStage", "accStage", "evaStage"])
                    getPkmn(i)[j] = 0;
            getPkmn(false).status = "";
            if (getPkmn(true).status == "tox")
                getPkmn(true).status = "psn";
        }
    }, {
        "name": "headbutt",
        "type": "normal",
        ...
        "effect": function () {
            if (!getType(false).includes("grass"))
                addTempEffect(false, "leech seed", Infinity, 1);
        }
    }, {
        "name": "leer",
        "type": "normal",
